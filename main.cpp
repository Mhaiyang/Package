...
#define  MAX_GENERATION   100   // 遗传的最大代数  
...
public:
	...
	void PrintOptimal()
	{
		std::cout << "The optimal solution is in " + Int_2_String(m_best_entity.generation_id) + " generation." << std::endl;
...
private:
	...
	void Init_Goods()
	{
		...
		for (int i = 0; i < m_goods_num; i++)
		{
			std::cout << "物品" << i << "  价值：" << m_goods[i].value << "  体积：" << m_goods[i].volume << "  重量：" << m_goods[i].weight << "\n";
		}
	}
	...
	void RecordOptimalEntity(int gid)
	{
		...
		std::cout << "Group Number: " + Int_2_String(m_group.size()) + "  Generation: " + Int_2_String(gid) +"th  Value: " + Int_2_String(en.fitness) + "  Volume: "+Int_2_String(en.sum_volume)
			+ "  Weight: " + Int_2_String(en.sum_weight) + "\nSolution: ";
